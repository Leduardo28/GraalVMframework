    ...
    if isinstance(stmt, javalang.tree.StatementExpression):
        if isinstance(stmt.expression, javalang.tree.MethodInvocation):
            if stmt.expression.qualifier == 'System.out' and stmt.expression.member == 'println':
                ...
                resultado.append(f"{indent}print({args})")
            else:
                args = ', '.join([traducir_expr(arg) for arg in stmt.expression.arguments])
        ...
        for declarator in stmt.declarators:
            ...
            resultado.append(f"{indent}{nombre} = {valor}")
        ...
        if isinstance(init, javalang.tree.VariableDeclaration):
            ...
            resultado.append(f"{indent}for {var} in range({inicio}, {fin}):")
            resultado.extend(traducir_bloque(stmt.body, nivel + 1))
        else:
            resultado.append(f"{indent}# For no soportado o no estándar")
...
def traducir_clase_y_metodos(tree):
    ...
    for tipo in tree.types:
        if isinstance(tipo, javalang.tree.ClassDeclaration):
            resultado.append(f"class {tipo.name}:")
            tiene_metodos = False
            ...
            for miembro in tipo.body:
                if isinstance(miembro, javalang.tree.MethodDeclaration):
                    ...
                    resultado.append(f"    def {nombre_metodo}():")
                    resultado.extend(traducir_bloque(miembro.body, nivel=2))

            if not tiene_metodos:
                resultado.append("    pass")

    return resultado

def traducir_archivo_java_a_python(ruta_archivo_java):
...
def guardar_como_py(nombre_entrada, codigo_py):
    nombre_salida = os.path.splitext(nombre_entrada)[0] + ".py"
...
if __name__ == "__main__":
    archivo_java = "HolaMundo.java"  # Cambia por tu archivo .java
    codigo_python = traducir_archivo_java_a_python(archivo_java)
    print("📤 Código traducido:\n")
    print(codigo_python)
    guardar_como_py(archivo_java, codigo_python)
